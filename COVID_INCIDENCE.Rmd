...
predicted.incidence.increase.4 <- 
  10^predict(fit.incidence.increase.4,
             data.frame(Date=seq.Date(ymd("2021-06-29"),Sys.Date(),by = "1 day"),interval = "prediction"),
        interval = "prediction") %>%
  tbl_df() %>%
  dplyr::mutate(Date=seq.Date(ymd("2021-06-29"),Sys.Date(),by = "1 day"))
...
df %>%
  ...
  scale_x_date(date_breaks = "1 month",limits=c(dmy("01-02-2020"),Sys.Date()),date_labels = "%Y %b") +
...
read.csv(paste0(temp_dir,"/Test_pos_over_time.csv"),sep=";") %>% 
  ...
  scale_x_date(date_breaks = "1 month",limits=c(dmy("01-02-2020"),Sys.Date()+days(7)),date_labels = "%b %Y") +
    scale_y_continuous(limits=c(0,NA),breaks=seq(0,100000000,by=5000000),expand=c(0,0),labels=scales::comma) +
  theme_coviddk +
...
read.csv(paste0(temp_dir,"/Test_pos_over_time.csv"),sep=";") %>% 
  ...
  scale_y_continuous(limits=c(0,NA),labels = scales::percent_format(accuracy = 0.1),expand=c(0,0),breaks = seq(0,1,by=0.001)) +
