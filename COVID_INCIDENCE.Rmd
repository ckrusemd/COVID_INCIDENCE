...
This report was compiled `r compile_time `.
...
ggplot(df.antigen_pcr,aes(x=Dato,y=AG_testede)) +
  ...
  scale_y_continuous(labels=scales::comma,expand=c(0,0),breaks=seq(0,5000000,by=25000),limits=c(0,NA)) +
...
colnames(df.confusion_matrix) <- c("Predicted Positive","Predicted Negative")
row.names(df.confusion_matrix) <- c("Observed Positive","Observed Negative")
  df.confusion_matrix %>% 
    knitr::kable()
...
read.csv(paste0(temp_dir,"/Test_pos_over_time.csv"),sep=";") %>% 
  ...
  geom_col() +
  labs(x="Date",y="Number of Daily PCR Tests") +
...
Track the time lag between surges in cases, admissions and deaths.
...
ggplot(  
  ...
  scale_y_continuous(limits=c(0,NA),breaks = seq(0,5800000,by=100000),labels = scales::unit_format(unit = "K",scale = 1e-3,big.mark = ",")) +
...
df_calendar = bind_rows(
  data.frame(Group="Gruppe 1",Date=as.Date("2020521", "%Y%U%u")),
  data.frame(Group="Gruppe 2",Date=as.Date("2021011", "%Y%U%u")),
  data.frame(Group="Gruppe 3",Date=as.Date("2021051", "%Y%U%u")),
  data.frame(Group="Gruppe 4 Pt 1",Date=as.Date("2020521", "%Y%U%u")),
  data.frame(Group="Gruppe 4 Pt 2",Date=as.Date("2021061", "%Y%U%u")),
  data.frame(Group="Gruppe 5",Date=as.Date("2021011", "%Y%U%u")),
  data.frame(Group="Gruppe 6",Date=as.Date("2021011", "%Y%U%u")),
  data.frame(Group="Gruppe 7",Date=as.Date("2021111", "%Y%U%u")),
  data.frame(Group="Gruppe 8",Date=as.Date("2021131", "%Y%U%u")),
  data.frame(Group="Gruppe 9",Date=as.Date("2021141", "%Y%U%u")),
  data.frame(Group="Gruppe 10a",Date=as.Date("2021171", "%Y%U%u")),
  data.frame(Group="Gruppe 10b",Date=as.Date("2021181", "%Y%U%u")),
  data.frame(Group="Gruppe 10c",Date=as.Date("2021191", "%Y%U%u")),
  data.frame(Group="Gruppe 10d1",Date=as.Date("2021211", "%Y%U%u")),
  data.frame(Group="Gruppe 10d2",Date=as.Date("2021241", "%Y%U%u")),
  data.frame(Group="Gruppe 10d3",Date=as.Date("2021261", "%Y%U%u")),
  data.frame(Group="Gruppe 10d4",Date=as.Date("2021291", "%Y%U%u"))
)
...
ggplot(df.predicted,aes(x=date,y=fit)) +
  ...
  geom_text(mapping = aes(x = Date,
                          y = 1000000,
                          label = Group),
            data = df_calendar,size=4,angle=-45,hjust=-0.1)
